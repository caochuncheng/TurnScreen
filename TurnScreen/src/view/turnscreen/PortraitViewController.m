...
- (void)initView:(CGRect)frame
{
    ...
    [btn setTitle:NSLocalizedString(@"btn.turn.landscape.name", @"转横屏") forState:UIControlStateNormal];
...
-(void)onClickButton:(UIButton *)button
{
    if(button.tag == kAppEventTurn){
        if ([self.appEventDelegate respondsToSelector:@selector(onClickEvent:eventId:obj:)]){
            [self.appEventDelegate onClickEvent:kAppViewController eventId:kAppEventChanged obj:@"LandscapeViewController"];
        }
        return;
    }
}
